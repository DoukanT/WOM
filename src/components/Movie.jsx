...
const Movie = ({ item }) => {
  ...
  const requestCast= "https://api.themoviedb.org/3/movie/"+item?.id+"/credits?api_key="+requests.key+"&language=en-US"
  ...
  if (!cast?.cast) {
    ...
    return (
      ...
      <div className='absolute top-0 left-0 w-full h-full hover:bg-black/80 opacity-0 hover:opacity-100 text-white'>
          ...
          <p>
          {liked ? (
            <FaHeart onClick={()=> unlikeMovie(item?.id)} className='h-6 w-6 absolute top-5 left-3 text-gray-300' />
          ) : (
            <FaRegHeart onClick={likeMovie} className='h-6 w-6 absolute top-5 left-3 text-gray-300' />
          )}
        </p> 

        <p>
          {later ? (
            <MdOutlineCheck onClick={()=> unlaterMovie(item?.id)} className='h-6 w-6 absolute top-5 left-10 text-gray-300' />
          ) : (
